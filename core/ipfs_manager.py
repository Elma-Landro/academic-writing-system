...
class IPFSManager:
    ...
    def __init__(self):
        """Initialise le gestionnaire IPFS avec support multi-provider."""
        # Configuration depuis les secrets
        self.fileverse_api_key = self._get_secret("ipfs", "FILEVERSE_API_KEY", "FILEVERSE_API_KEY")
        self.pinata_api_key = self._get_secret("ipfs", "PINATA_API_KEY", "PINATA_API_KEY") 
        self.pinata_secret = self._get_secret("ipfs", "PINATA_SECRET", "PINATA_SECRET")

    def _get_secret(self, section: str, key: str, env_var: str) -> Optional[str]:
        """Récupère un secret depuis Streamlit ou les variables d'environnement."""
        try:
